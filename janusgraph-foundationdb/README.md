...
|`storage.fdb.version`|The FoundationDB client version.|`6.0.0`|
...
|`storage.fdb.tls-certificate-file-path`|The location of the file to load the local TLS certificates from.|`null`|
|`storage.fdb.tls-key-file-path`|The location of the file to load the private TLS key from.|`null`|
|`storage.fdb.tls-ca-file-path`|The location of the file containing the CA certificates to trust.|`null`|
|`storage.fdb.tls-verify-peers`|The byte-string for the verification of peer certificates and sessions.|`null`|
...
See the FoundationDB documentation on [Transport Layer Security - Configuring TLS](https://apple.github.io/foundationdb/tls.html) for details on the `tls-certificate-file-path`, `tls-key-file-path`, `tls-key-file-path`, and `tls-ca-file-path` purpose and expected values.
